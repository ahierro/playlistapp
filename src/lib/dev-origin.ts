 ...
 * IP (`http://127.0.0.1:3000/...`). The Google (YouTube Music) session goes
 * through the same wrapper so both providers use one registered origin. But the Next 16 dev server normalizes
...
export function withDevOrigin(handler: Handler): Handler {
